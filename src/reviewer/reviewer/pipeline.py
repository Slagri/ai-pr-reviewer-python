...
from reviewer.reviewer.orchestrator import run_orchestrated_review
from reviewer.reviewer.prompts import SYSTEM_PROMPT, build_user_prompt
from reviewer.reviewer.tools import ToolExecutor

# PRs with more than this many files use multi-agent mode
MULTI_AGENT_FILE_THRESHOLD = 5
...
async def _execute_review(
    *,
    ...
    # Step 5: Choose review strategy — multi-agent for larger PRs
    use_multi_agent = len(diffs) >= MULTI_AGENT_FILE_THRESHOLD
    log.info("review strategy", mode="multi-agent" if use_multi_agent else "single-agent")

    if use_multi_agent:
        review_data, trace = await run_orchestrated_review(
